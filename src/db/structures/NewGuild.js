...
} = require("../managers/GuildSoundboardSoundManager");
...
class Guild extends AnonymousGuild {
  ...
  _patch(data) {
    ...
    if ("incidents_data" in data) {
       ...
       */
      this.incidentsData =
        data.incidents_data && _transformAPIIncidentsData(data.incidents_data);
    } else {
  ...
  async fetchWidgetSettings() {
    const data = await this.client.rest.get(
      Routes.guildWidgetSettings(this.id)
    );
  ...
  async edit({
    ...
    const data = await this.client.rest.patch(Routes.guild(this.id), {
      body: {
        ...
        afk_channel_id:
          afkChannel && this.client.channels.resolveId(afkChannel),
        afk_timeout: afkTimeout,
        icon: icon && (await resolveImage(icon)),
        owner_id: owner && this.client.users.resolveId(owner),
        splash: splash && (await resolveImage(splash)),
        discovery_splash:
          discoverySplash && (await resolveImage(discoverySplash)),
        banner: banner && (await resolveImage(banner)),
        system_channel_id:
          systemChannel && this.client.channels.resolveId(systemChannel),
        system_channel_flags:
          systemChannelFlags === undefined ? undefined : (
            SystemChannelFlagsBitField.resolve(systemChannelFlags)
          ),
        rules_channel_id:
          rulesChannel && this.client.channels.resolveId(rulesChannel),
        public_updates_channel_id:
          publicUpdatesChannel &&
          this.client.channels.resolveId(publicUpdatesChannel),
        preferred_locale: preferredLocale,
        premium_progress_bar_enabled: premiumProgressBarEnabled,
        safety_alerts_channel_id:
          safetyAlertsChannel &&
          this.client.channels.resolveId(safetyAlertsChannel)
      },
  ...
  async editOnboarding(options) {
    const newData = await this.client.rest.put(
      Routes.guildOnboarding(this.id),
      {
        body: {
          prompts: options.prompts?.map((prompt) => ({
            ...
            options: prompt.options.map((option) => {
              ...
              return {
                id: option.id,
                channel_ids: option.channels?.map((channel) =>
                  this.channels.resolveId(channel)
                ),
                role_ids: option.roles?.map((role) =>
                  this.roles.resolveId(role)
                ),
                title: option.title,
          ...
          default_channel_ids: options.defaultChannels?.map((channel) =>
            this.channels.resolveId(channel)
          ),
  ...
  async editWelcomeScreen(options) {
    ...
    const patchData = await this.client.rest.patch(
      Routes.guildWelcomeScreen(this.id),
      {
        body: {
          welcome_channels,
          description,
  ...
  _sortedChannels(channel) {
    ...
    return discordSort(
      this.channels.cache.filter(
        ({ parentId, type }) =>
          types.includes(type) &&
          (channelIsCategory || parentId === channel.parentId)
      )
    );
  }
}
